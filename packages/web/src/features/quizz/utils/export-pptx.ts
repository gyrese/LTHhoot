...
const applyBackground = async (
  ...
  if (bg.type === "image") {
    ...
    if (data) {
      slide.addImage({
        data, x: 0, y: 0, w: SLIDE_W, h: SLIDE_H,
...
const renderElements = async (slide: pptxgen.Slide, elements: SlideElement[]) => {
  for (const el of elements) {
    ...
    if (el.type === "text") {
      ...
      slide.addText(te.text, {
        x: cx(te.x), y: cy(te.y), w: cw(te.width), h: ch(te.height),
      ...
      if (data) {
        slide.addImage({
          data,
          x: cx(ie.x), y: cy(ie.y), w: cw(ie.width), h: ch(ie.height),
          transparency: transp,
          rotate: el.rotation,
        })
      }
      ...
      const shapeMap: Record<string, pptxgen.SHAPE_NAME> = {
        rect: "rect", circle: "ellipse", triangle: "triangle", star: "star5",
      }
      slide.addShape((shapeMap[se.shapeType] ?? "rect") as pptxgen.SHAPE_NAME, {
        x: cx(se.x), y: cy(se.y), w: cw(se.width), h: ch(se.height),
        fill: { color: cssColorToHex(se.fill) ?? "888888", transparency: transp },
        line: { width: 0 },
        rotate: el.rotation,
      })
    }
...
const addBadge = (slide: pptxgen.Slide, question: Question, index: number, total: number) => {
  ...
  slide.addText(`${TYPE_LABELS[question.type] ?? question.type}  ·  ${index} / ${total}`, {
    x: 0.4, y: 0.2, w: 2.4, h: 0.45,
    fontSize: 13, color: "FFFFFF", bold: true, align: "center",
  })
  slide.addText(`⏱ ${question.time}s`, {
    x: SLIDE_W - 2, y: 0.2, w: 1.6, h: 0.45,
    fontSize: 13, color: "DDDDDD", align: "right",
  })
}
...
const addQuestionSlide = async (
  ...
  if (question.media?.url && question.media.type === "image") {
    ...
    if (data) {
      hasMedia = true
      slide.addImage({
        data, x: SLIDE_W / 2, y: 1.0, w: SLIDE_W / 2 - 0.5, h: SLIDE_H - 1.5,
...
const addAnswersSlide = async (
  ...
  if (question.question) {
    slide.addText(question.question, {
      x: 0.5, y: 0.85, w: SLIDE_W - 1, h: 0.9,
      fontSize: 20, bold: true, color: "FFFFFF", wrap: true,
...
const addMcqAnswers = async (slide: pptxgen.Slide, q: McqQuestion, y: number, h: number) => {
  ...
  q.answers.forEach((answer, i) => {
    ...
    if (isCorrect) {
      slide.addText("✓", {
        x: x + boxW - 0.7, y: boxY + 0.05, w: 0.6, h: 0.5,
        fontSize: 18, color: "FFFFFF", align: "center", bold: true,
      })
    }

    slide.addText(`${ANSWER_SHAPES[i]}  ${answer}`, {
      x: x + 0.2, y: boxY, w: boxW - 0.9, h: boxH,
      fontSize: 18, color: "FFFFFF", bold: true, wrap: true, valign: "middle",
    })
  })
}

const addTrueFalseAnswers = (slide: pptxgen.Slide, q: TrueFalseQuestion, y: number, h: number) => {
  const options = [
...
const addOpenAnswers = (slide: pptxgen.Slide, q: OpenQuestion, y: number, h: number) => {
  ...
  slide.addText("Réponses acceptées :", {
    x: 0.8, y: y + 0.2, w: 8, h: 0.4,
    fontSize: 14, color: "BBBBBB", italic: true,
  })
  slide.addText(q.correctAnswers.join("  /  "), {
    x: 0.8, y: y + 0.7, w: SLIDE_W - 1.6, h: h - 1,
    fontSize: 22, color: "7ed56f", bold: true, wrap: true,
  })
}
...
const addDateAnswer = (slide: pptxgen.Slide, q: DateQuestion, y: number, h: number) => {
  ...
  slide.addText("Année correcte", { x, y: y + 0.2, w: boxW, h: 0.5, fontSize: 14, color: "CCDDFF", align: "center", italic: true })
...
const addSliderAnswer = (slide: pptxgen.Slide, q: SliderQuestion, y: number, _h: number) => {
  ...
  slide.addText(String(q.max), { x: barX + barW - 1.5, y: barY + 0.55, w: 1.5, h: 0.4, fontSize: 14, color: "AAAAAA", align: "right" })
  slide.addText(`Valeur : ${q.correctValue}${q.tolerance > 0 ? ` ± ${q.tolerance}` : ""}`, {
    x: 0, y: barY + 1.1, w: SLIDE_W, h: 0.5,
    fontSize: 18, color: "7ed56f", bold: true, align: "center",
  })
}

const addPuzzleAnswers = (slide: pptxgen.Slide, q: PuzzleQuestion, y: number, h: number) => {
  ...
  q.items.forEach((item, i) => {
    ...
    slide.addText(`${i + 1}.  ${item}`, {
      x: 0.8, y: itemY, w: SLIDE_W - 1.6, h: itemH,
      fontSize: 18, color: "FFFFFF", bold: true, valign: "middle",
    })
  })
}

const addDropPinAnswer = async (slide: pptxgen.Slide, q: DropPinQuestion, y: number, h: number) => {
  if (q.pinImage) {
  ...
  const labels = correctZones.map((z) => z.label).filter(Boolean).join(", ")

  if (labels) {
    slide.addShape("rect", {
      x: 0.4, y: y + h - 0.55, w: SLIDE_W - 0.8, h: 0.5,
      fill: { color: "26890c" }, line: { width: 0 }, rectRadius: 0.1,
    })
    slide.addText(`Zone(s) correcte(s) : ${labels}`, {
      x: 0.7, y: y + h - 0.55, w: SLIDE_W - 1.4, h: 0.5,
      fontSize: 14, color: "FFFFFF", bold: true, valign: "middle",
    })
  }
}
...
const addCoverSlide = async (prs: pptxgen, quizz: Quizz) => {
  ...
  if (quizz.salonImage) {
    ...
    if (data) {
      slide.addImage({ data, x: 0, y: 0, w: SLIDE_W, h: SLIDE_H, transparency: 40 })
    }
  }
  ...
  if (quizz.description) {
    slide.addText(quizz.description, {
      x: 2, y: 4.4, w: SLIDE_W - 4, h: 1,
      fontSize: 22, color: "CCCCCC", align: "center", italic: true,
    })
  }

  slide.addText(`${quizz.questions.length} question${quizz.questions.length > 1 ? "s" : ""}`, {
    x: SLIDE_W / 2 - 2, y: 5.7, w: 4, h: 0.6,
    fontSize: 18, color: "ff6b35", align: "center", bold: true,
  })
}
