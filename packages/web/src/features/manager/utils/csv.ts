...
export const downloadGameResultCSV = (result: GameResult) => {
  ...
  const rows = result.players.map((player) => {
    ...
    result.questions.forEach((q) => {
      const pAnswer = q.playerAnswers.find((pa) => pa.playerName === player.username)
  ...
  link.setAttribute("download", `results_${result.subject.replace(/\s+/g, "_")}_${new Date(result.date).getTime()}.csv`)
