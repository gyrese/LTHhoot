...
export function RemoteControl({ gameId }: { gameId: string }) {
  ...
  // Auth
  const [password, setPassword] = useState(() => sessionStorage.getItem("rc_pwd") ?? "")
  ...
  useEvent(EVENTS.MANAGER.SUCCESS_RECONNECT, ({ gameId: gId, status: s, players: p, currentQuestion }) => {
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-gradient-to-b from-slate-950 via-gray-900 to-black overflow-hidden">
      {/* Ambiance lumineuse */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -top-20 left-1/3 h-72 w-72 animate-pulse rounded-full bg-orange-500/6 blur-3xl" />
        <div className="absolute bottom-32 right-1/4 h-56 w-56 animate-pulse rounded-full bg-violet-500/6 blur-3xl [animation-delay:1.5s]" />
      </div>
...
function AuthScreen({
  password, setPassword, error, isLoading, isConnected, onSubmit,
}: {
  password: string
  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center gap-8 bg-gradient-to-b from-slate-950 to-black p-6">
      <div className="text-center">
        <img src={logo} alt="LTNHoot!" className="mx-auto h-16 w-auto object-contain" />
        <div className="mt-2 text-xs uppercase tracking-[0.3em] text-white/30">Télécommande</div>
      </div>

      <div className="w-full max-w-xs rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-md">
        <div className="mb-5 flex items-center gap-2">
          <div className={clsx("h-2 w-2 rounded-full", isConnected ? "bg-green-400" : "animate-pulse bg-red-400")} />
          <span className="text-xs text-white/40">{isConnected ? "Serveur connecté" : "Connexion..."}</span>
        </div>
      ...
      <p className="max-w-xs text-center text-xs text-white/20">
        Ouvrez cette page sur votre téléphone pendant que le jeu tourne sur l'écran principal.
      </p>
    </div>
  )
}
...
function RemoteHeader({
  isConnected, questionStates, statusName, inviteCode,
}: {
  isConnected: boolean
  ...
  return (
    <header className="relative z-20 flex items-center gap-3 border-b border-white/8 bg-black/40 px-4 py-3 backdrop-blur-xl">
      ...
      <div className="flex items-center gap-2">
        {inviteCode && (
          <button
            onClick={() => { setCodeVisible((v) => !v); if (!codeVisible) copyCode() }}
    ...
    <div className="flex flex-col gap-4 p-4">
      ...
      {correctAnswers.length > 0 && (
        <div className="flex flex-wrap gap-2 rounded-2xl border border-green-500/20 bg-green-500/5 p-3">
          <p className="w-full text-xs font-bold uppercase tracking-widest text-green-400/70">Réponses valides</p>
          {correctAnswers.map((ca) => (
            <span key={ca} className="flex items-center gap-1 rounded-full border border-green-500/30 bg-green-500/15 px-3 py-1 text-xs font-semibold text-green-300">
      ...
      <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/40 backdrop-blur-sm">
        ...
        <p className="px-4 pb-1 pt-2.5 text-[11px] text-white/30">Appuyer pour valider une réponse</p>
        <div className="flex flex-col gap-1.5 p-3">
          {unique.map((answer) => {
            const count = answers.filter(
              (a) => a.text.trim().toLowerCase() === answer.text.trim().toLowerCase(),
            ).length
            const players = answers
              .filter((a) => a.text.trim().toLowerCase() === answer.text.trim().toLowerCase())
              .map((a) => a.playerName)
            ...
            return (
              <button
                ...
                className={clsx(
                  ...
                  validating === answer.text && !answer.isCorrect && "opacity-50",
                )}
              >
                {answer.isCorrect && <IconCheck className="h-4 w-4 shrink-0 text-green-400" />}
                <div className="min-w-0 flex-1">
                  <p className={clsx("truncate text-sm font-bold", answer.isCorrect ? "text-green-300" : "text-white")}>
                    {answer.text}
                  </p>
                  <p className="truncate text-xs text-white/35">{players.join(", ")}</p>
                </div>
                {count > 1 && (
...
function RoomPanel({ data, players }: { data: Record<string, unknown>; players: Player[] }) {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      <button
        onClick={copyCode}
        className="group w-full rounded-2xl border border-white/10 bg-black/40 p-5 text-center backdrop-blur-sm transition-all active:scale-98 hover:border-orange-500/20"
      >
        <p className="mb-2 text-xs uppercase tracking-widest text-white/30">Code d'invitation</p>
        <p className="font-mono text-5xl font-black tracking-[0.3em] text-orange-400 transition-colors group-hover:text-orange-300">
          {inviteCode}
        </p>
        <p className="mt-2 text-xs text-white/20">{copied ? "✓ Copié !" : "Appuyer pour copier"}</p>
      </button>
      ...
      <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/30 px-5 py-3.5 backdrop-blur-sm">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-white/30">connecté{players.length !== 1 ? "s" : ""}</p>
        </div>
        {/* Avatars empilés */}
      ...
      {players.length > 0 ? (
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/30 backdrop-blur-sm">
          <div className="border-b border-white/5 px-4 py-2.5">
            <p className="text-xs font-semibold uppercase tracking-wider text-white/40">Joueurs</p>
          </div>
          <div className="max-h-64 divide-y divide-white/5 overflow-y-auto">
            {players.map((player) => (
              <div key={player.id} className="flex items-center gap-3 px-4 py-2.5">
                ...
                <span className="flex-1 truncate text-sm font-medium text-white">{player.username}</span>
                <div className={clsx("h-1.5 w-1.5 rounded-full", player.connected ? "bg-green-400" : "bg-white/20")} />
              </div>
            ))}
          </div>
        </div>
      ) : (
        <p className="py-8 text-center text-sm text-white/20">En attente des joueurs...</p>
      )}
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      <div className="flex flex-col items-center gap-4 rounded-2xl border border-white/10 bg-black/40 p-6 backdrop-blur-sm">
        <div className="relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-4xl font-black text-white tabular-nums">{answerCount}</span>
      ...
      {players.length > 0 && (
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/30 backdrop-blur-sm">
          <div className="border-b border-white/5 px-4 py-2.5">
            <p className="text-xs font-semibold uppercase tracking-wider text-white/35">Suivi joueurs</p>
          </div>
              ...
              return (
                <div key={player.id} className="flex items-center gap-3 px-4 py-2.5">
                  <div className={clsx(
                    ...
                    hasAnswered ? "bg-green-400" : "animate-pulse bg-white/15",
                  )} />
                  <span className="flex-1 truncate text-sm text-white">{player.username}</span>
                  {hasAnswered
...
function ResponsesPanel({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      {/* Question */}
      <div className="rounded-2xl border border-white/10 bg-black/40 px-4 py-3 backdrop-blur-sm">
        <p className="mb-0.5 text-xs text-white/30">{QUESTION_TYPE_LABELS[type] ?? type}</p>
        <p className="line-clamp-2 text-sm font-semibold text-white">{question}</p>
      </div>
      ...
      {type === "mcq" && answers.length > 0 && (
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/40 backdrop-blur-sm">
          <div className="border-b border-white/5 px-4 py-2.5">
            <p className="text-xs font-semibold uppercase tracking-wider text-white/35">Réponses</p>
          </div>
                ...
                <div key={i} className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="mb-1 flex items-center justify-between gap-2">
                      <span className="truncate text-xs text-white">{answer}</span>
                      <div className="flex shrink-0 items-center gap-1">
                        <span className="text-xs font-bold text-white tabular-nums">{count}</span>
                        {isCorrect && <IconCheck className="h-3.5 w-3.5 text-green-400" />}
                      </div>
                    </div>
                    <div className="h-1.5 overflow-hidden rounded-full bg-white/8">
                      <div
                        className={clsx("h-full rounded-full transition-all duration-700", isCorrect ? "bg-green-500" : color.bar)}
                        style={{ width: `${pct}%`, opacity: isCorrect ? 1 : 0.6 }}
                      />
                    </div>
                  </div>
                  <span className="w-8 shrink-0 text-right text-xs text-white/35 tabular-nums">{pct}%</span>
                </div>
              )
            })}
          </div>
            ...
            {["Vrai", "Faux"].map((label, i) => {
              ...
              return (
                <div key={i} className={clsx(
                  ...
                  isCorrect ? "border-green-500/30 bg-green-500/10" : "border-white/8 bg-white/4",
                )}>
                  <p className="text-2xl font-black text-white tabular-nums">{count}</p>
                  <p className="text-xs text-white/50">{label}</p>
                  <p className="text-xs font-semibold text-white/35 tabular-nums">{pct}%</p>
                  {isCorrect && <IconCheck className="mx-auto mt-1.5 h-4 w-4 text-green-400" />}
                </div>
              )
            })}
          </div>
      ...
      {type === "open" && (
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/40 backdrop-blur-sm">
          <div className="border-b border-white/5 px-4 py-2.5">
            <p className="text-xs font-semibold uppercase tracking-wider text-white/35">Réponses acceptées</p>
          </div>
          <div className="flex flex-col gap-2 p-3">
            {correctAnswers.map((ans, i) => (
              <div key={i} className="flex items-center gap-2 rounded-xl border border-green-500/20 bg-green-500/8 px-3 py-2.5">
                <IconCheck className="h-4 w-4 shrink-0 text-green-400" />
                <span className="text-sm font-semibold text-green-300">{ans}</span>
              </div>
            ))}
          </div>
          <div className="border-t border-white/5 px-4 py-2.5">
            <p className="text-xs text-white/25">{total} réponse{total !== 1 ? "s" : ""} reçue{total !== 1 ? "s" : ""}</p>
          </div>
        </div>
      )}
      ...
      {(type === "date" || type === "slider") && (
        <div className="rounded-2xl border border-white/10 bg-black/40 p-5 backdrop-blur-sm">
          <p className="mb-1 text-xs uppercase tracking-wider text-white/35">Bonne réponse</p>
          <p className="text-4xl font-black text-green-400 tabular-nums">
            {type === "date" ? String(correctYear ?? "—") : String(correctValue ?? "—")}
          </p>
          <p className="mt-2 text-xs text-white/25">{total} réponse{total !== 1 ? "s" : ""}</p>
        </div>
      )}
      ...
      {type === "puzzle" && (
        <div className="rounded-2xl border border-white/10 bg-black/40 p-4 backdrop-blur-sm">
          <p className="mb-2 text-xs uppercase tracking-wider text-white/35">Ordre correct</p>
          <div className="flex flex-col gap-2">
            {((data.items as string[]) ?? []).map((item, i) => (
              <div key={i} className="flex items-center gap-3 rounded-xl border border-white/8 bg-white/5 px-3 py-2">
                <span className="text-xs font-black text-orange-400">{i + 1}</span>
      ...
      {type === "drop_pin" && (
        <div className="rounded-2xl border border-white/10 bg-black/40 p-5 backdrop-blur-sm">
          <p className="mb-1 text-xs uppercase tracking-wider text-white/35">Zones de marquage</p>
          <div className="flex flex-col gap-2">
            {((data.zones as { x: number; y: number; radius: number }[]) ?? []).map((z, i) => (
              <div key={i} className="flex items-center gap-2 text-sm text-white/60">
                <div className="h-2 w-2 rounded-full bg-orange-500" />
                <span>Zone {i + 1} : {Math.round(z.x)}%, {Math.round(z.y)}% (r={Math.round(z.radius)}px)</span>
              </div>
            ))}
          </div>
          <p className="mt-2 text-xs text-white/25">{total} réponse{total !== 1 ? "s" : ""}</p>
...
function SolutionDisplay({ data }: { data: Record<string, unknown> }) {
  ...
  const hasSolution = solutions.length > 0 || correctAnswers.length > 0 || correctYear !== undefined || correctValue !== undefined || items.length > 0 || zones.length > 0
  ...
  return (
    <div className="rounded-2xl border border-green-500/20 bg-green-500/5 p-4 backdrop-blur-sm">
      <div className="mb-2 flex items-center gap-2">
        ...
        <span className="text-xs font-bold uppercase tracking-widest text-green-400/80">Solution</span>
      </div>
      ...
      <div className="text-sm font-semibold text-white">
        {type === "mcq" && (
          <div className="flex flex-wrap gap-2">
            {solutions.map(idx => (
              <span key={idx} className="rounded-lg bg-green-500/20 px-2 py-1 text-green-300">
        ...
        {type === "true_false" && (
          <span className="text-green-300">{solutions[0] === 0 ? "Vrai" : "Faux"}</span>
        )}
        {type === "open" && (
          <div className="flex flex-wrap gap-2">
            {correctAnswers.map((ans, i) => (
              <span key={i} className="rounded-lg bg-green-500/20 px-2 py-1 text-green-300">{ans}</span>
            ))}
          </div>
        )}
        {type === "date" && <span className="text-2xl font-black text-green-300 tabular-nums">{correctYear}</span>}
        {type === "slider" && <span className="text-2xl font-black text-green-300 tabular-nums">{correctValue}</span>}
        {type === "puzzle" && (
...
function LeaderboardPanel({ data }: { data: Record<string, unknown> }) {
  type LeaderRow = { id: string; username: string; points: number; roundPoints?: number }
      ...
      {roundLB.length > 0 && (
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/40 backdrop-blur-sm">
          <div className="flex items-center gap-2 border-b border-white/5 px-4 py-2.5">
            ...
            <p className="text-xs font-semibold uppercase tracking-wider text-white/45">Cette manche</p>
          </div>
          <div className="flex flex-col gap-0 divide-y divide-white/5 px-3 py-2">
            {roundLB.slice(0, 5).map((player, i) => (
              <div key={player.id} className="flex items-center gap-3 py-2.5">
                ...
                <span className="flex-1 truncate text-sm font-semibold text-white">{player.username}</span>
                <span className="text-sm font-bold text-orange-400 tabular-nums">+{player.roundPoints ?? 0}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {leaderboard.length > 0 && (
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/40 backdrop-blur-sm">
          <div className="flex items-center gap-2 border-b border-white/5 px-4 py-2.5">
            ...
            <p className="text-xs font-semibold uppercase tracking-wider text-white/40">Classement général</p>
          </div>
            ...
            {leaderboard.slice(0, 5).map((player, i) => (
              ...
              >
                <div className={clsx(
                  ...
                  i === 0 && "border-yellow-500/40 bg-yellow-500/15 text-yellow-300",
                  i === 1 && "border-slate-400/30 bg-slate-400/10 text-slate-300",
                  i === 2 && "border-orange-600/30 bg-orange-700/10 text-orange-400",
                  i >= 3 && "border-white/10 bg-white/5 text-white/35",
                )}>
                  {i + 1}
                </div>
                <span className="flex-1 truncate text-sm font-semibold text-white">{player.username}</span>
                <span className="text-sm font-bold text-white tabular-nums">{player.points.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {top.length > 0 && (
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/40 backdrop-blur-sm">
          <div className="border-b border-white/5 px-4 py-2.5">
            <p className="text-xs font-semibold uppercase tracking-wider text-white/40">Podium</p>
          </div>
          <div className="flex flex-col gap-2.5 p-3">
            {top.map((player, i) => (
              <div key={player.username} className={clsx(
                ...
                <span className="flex-1 truncate font-bold text-white">{player.username}</span>
                <span className="font-black text-white tabular-nums">{player.points.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
...
function PlayersPanel({
  players, statusName, answerCount, kickTargetId, setKickTargetId, onKick,
}: {
  players: Player[]
  statusName: Status | undefined
  answerCount: number
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      ...
      {players.length === 0 ? (
        <p className="py-12 text-center text-sm text-white/25">Aucun joueur connecté</p>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/30 backdrop-blur-sm">
          <div className="divide-y divide-white/5">
            {players.map((player, i) => {
              const isTarget = kickTargetId === player.id
              const hasAnswered = statusName === STATUS.SELECT_ANSWER && i < answerCount
              ...
              return (
                <div
                  ...
                  className={clsx(
                    ...
                    isTarget ? "bg-red-500/10" : "hover:bg-white/3 active:bg-white/5",
                  )}
                >
                  {/* Avatar */}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-white/35 tabular-nums">{player.points.toLocaleString()} pts</p>
                  </div>
                  ...
                  {statusName === STATUS.SELECT_ANSWER && (
                    <div className={clsx(
                      ...
                      hasAnswered ? "bg-green-400" : "animate-pulse bg-white/20",
                    )} />
                  )}
                  ...
                  {isTarget ? (
                    ...
                    <svg className="h-4 w-4 shrink-0 text-white/15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="mx-auto flex max-w-[430px] items-center gap-3">
        ...
        <button
          ...
          className={clsx(
            ...
            activeTab === "jeu" ? "bg-orange-500/15 text-orange-400" : "text-white/35 hover:text-white/60",
          )}
        >
        ...
        <button
          ...
          className={clsx(
            ...
            activeTab === "joueurs" ? "bg-orange-500/15 text-orange-400" : "text-white/35 hover:text-white/60",
          )}
        >
    ...
    >
      ...
      >
        <div className="mb-5 text-center">
          ...
          <p className="mt-1 font-semibold text-orange-400">{player.username}</p>
          <p className="mt-1 text-sm text-white/35">Cette action est définitive.</p>
        </div>
