...
try {
  ...
  console.error(
    "Échec de la création de la sauvegarde. Migration annulée par sécurité :",
    err.message,
  )
  process.exit(1)
}
...
try {
  ...
  console.log(
    "Sharp non disponible. Conservation du format d'origine (PNG/JPG).",
  )
}
...
for (const file of quizFiles) {
        ...
        if (matches) {
          ...
          if (sharp) {
            ...
            try {
              ...
              console.error(
                `[Sharp Error] Échec conversion WebP pour ${file}, fallback direct :`,
                err.message,
              )
            }
          }
  ...
  if (fileChanged) {
    ...
    console.log(
      `✅ ${file} : ${fileMigratedCount} image(s) migrée(s). Gain estimé : ~${mbSaved} Mo.`,
    )
...
console.log(
  `Espace disque / RAM libéré : ~ ${(totalSavedBytes / (1024 * 1024)).toFixed(2)} Mo`,
)
console.log("=========================================")
